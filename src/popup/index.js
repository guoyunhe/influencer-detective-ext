...
function render(state, tab) {
  ...
  if (result && Array.isArray(result.influencers) && result.influencers.length > 0) {
...
function renderSubmitForm(info, result) {
  ...
  const form = el('form', {}, [
    ...
    el('div', { class: 'row' }, [
      handleField('youtube', 'YouTube'),
      handleField('bilibili', 'Bilibili'),
    ]),
    el('div', { class: 'row' }, [
      handleField('twitter', 'Twitter'),
      handleField('instagram', 'Instagram'),
    ]),
